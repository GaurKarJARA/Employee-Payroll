 ...
 import java.util.stream.Collectors;
    ...
    public class AuthenticationService implements IAuthInterface {
        ...
        public String register(AuthUserDTO user) {
            try {
                List<AuthUser> l1 = userRepository.findAll().stream().filter(authuser ->
                        user.getEmail().equals(authuser.getEmail())).collect(Collectors.toList());
                if (l1.size() > 0) {
        ...
        public String login(LoginDTO user) {
            ...
            try {
                List<AuthUser> l1 = userRepository.findAll().stream().filter(authuser ->
                        authuser.getEmail().equals(user.getEmail())).collect(Collectors.toList());
                if (l1.size() == 0) {
        ...
        public AuthUserDTO forgotPassword(PassDTO pass, String email){
            ...
            log.error("user not found with email {} ,Exception : {}", email, e);
            }
            return null;
        }
